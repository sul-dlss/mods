...
module Mods
  ...
  LANG_ATTRIBS = %w[script transliteration].freeze
  LINKING_ATTRIBS = %w[xlink ID].freeze

  DATE_ATTRIBS = %w[encoding point keyDate qualifier].freeze
  ENCODING_ATTRIB_VALUES = %w[w3cdtf iso8601 marc edtf temper].freeze
  POINT_ATTRIB_VALUES = %w[start end].freeze
  KEY_DATE_ATTRIB_VALUES = ['yes'].freeze
  QUALIFIER_ATTRIB_VALUES = %w[approximate inferred questionable].freeze

  AUTHORITY_ATTRIBS = %w[authority authorityURI valueURI].freeze
  ...
  class Record
    def with_attributes(element, attributes = [], method_mappings: { 'type' => 'type_at', 'ID' => 'id_at' })
      attributes.each do |attr_name|
        ...
        element.send attr_method, path: "@#{attr_name}", accessor: ->(a) { a.text }
      end
    end
    ...
    def set_terminology_ns(mods_ng_xml)
      mods_ng_xml.set_terminology(namespaces: { 'm' => Mods::MODS_NS }) do |t|
        ...
        t._plain_name path: '(//m:name|//m:alternativeName)' do |n|
          ...
          n.role path: 'm:role' do |r|
            ...
            r.value path: '.', accessor: lambda { |role_node|
              ...
              val || role_node.roleTerm.select do |role_t|
                       role_t.type_at == 'code' && role_t.authority.match?(/marcrelator/)
                     end.first&.value
            }
          end
          ...
          n.display_value path: '.', single: true, accessor: lambda { |name_node|
            dv = ''
            dv = name_node.displayForm.text if name_node.displayForm && name_node.displayForm.text.size.positive?
            if dv.empty?
              if name_node.type_at == 'personal'
                if name_node.family_name.size.positive?
                  dv = name_node.given_name.size.positive? ? "#{name_node.family_name.text}, #{name_node.given_name.text}" : name_node.family_name.text
                elsif name_node.given_name.size.positive?
                  dv = name_node.given_name.text
                end
                if dv.empty? # no family or given name
                  dv = name_node.namePart.select { |np| np.type_at != 'date' && !np.text.empty? }.join(' ')
                else
                  first = true
                  name_node.namePart.each do |np|
          ...
          n.display_value_w_date path: '.', single: true, accessor: lambda { |name_node|
            dv = ''
            dv += name_node.display_value if name_node.display_value
            name_node.namePart.each do |np|
              dv += ", #{np.text}" if np.type_at == 'date' && !np.text.empty? && !dv.end_with?(np.text)
            end
            dv.sub(', ', '') if dv.start_with?(', ')
            dv.strip.empty? ? nil : dv.strip
          }
        end
        ...
        t._origin_info path: '//m:originInfo' do |n|
          n.as_object path: '.', accessor: ->(a) { Mods::OriginInfo.new(a) }
          # attributes
          with_attributes(n, Mods::LANG_ATTRIBS | %w[displayLabel])
          # child elements
          ...
          Mods::OriginInfo::DATE_ELEMENTS.each do |date_el|
            n.send date_el, path: "m:#{date_el}" do |d|
              d.as_object path: '.', accessor: ->(a) { Mods::Date.from_element(a) }
        ...
        t._subject path: '//m:subject' do |n|
          ...
          n.cartographics  path: 'm:cartographics' do |n1|
            ...
            Mods::Subject::CARTOGRAPHICS_CHILD_ELEMENTS.each do |elname|
          ...
          n.hierarchicalGeographic path: 'm:hierarchicalGeographic' do |n1|
            Mods::Subject::HIER_GEO_CHILD_ELEMENTS.each do |elname|
              n1.send elname, path: "m:#{elname}"
            end
            with_attributes(n1, Mods::AUTHORITY_ATTRIBS)
          end
