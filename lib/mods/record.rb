...
module Mods
  class Record
    ...
    NS_HASH = { 'm' => MODS_NS_V3 }.freeze

    ATTRIBUTES = %w[id version].freeze
      ...
      when Array
        ...
        messages.each do |msg|
          ...
          when String
            ...
            raise ArgumentError,
                  "term_values called with Array containing unrecognized class: #{msg.class}, #{messages.inspect}", caller
          end
        end
        nodes = obj
      else
    ...
    def personal_names
      @mods_ng_xml.personal_name.map(&:display_value)
    end
    ...
    def corporate_names
      @mods_ng_xml.corporate_name.map(&:display_value)
    end
    ...
    def languages
      result = []
      @mods_ng_xml.language.each do |n|
        # get languageTerm codes and add their translations to the result
        n.code_term.each do |ct|
          if ct.authority.match(/^iso639/)
            begin
              vals = ct.text.split(/[,|\ ]/).reject { |x| x.strip.length.zero? }
              vals.each do |v|
        ...
        n.text_term.each do |tt|
          val = tt.text.strip
          result << val if val.length.positive?
        end

        # add language values that aren't in languageTerm subelement
        result << n.text if n.languageTerm.size.zero?
      end
      result.uniq
    end
