...
describe 'Mods <subject> Element' do
  ...
  context 'with subterms for <name> child elements of <subject> element' do
    before(:all) do
    ...
    it 'is able to identify corporate names' do
      expect(@both_types_sub.corporate_name.namePart.map(&:text)).to eq(['Britton & Rey.'])
    end

    it 'is able to identify personal names' do
      expect(@both_types_sub.personal_name.namePart.map(&:text)).to eq(['Bridgens, R.P'])
      expect(@pers_name_sub.personal_name.displayForm.map(&:text)).to eq(['Edward VI , king of England, 1537-1553'])
    end

    it 'is able to identify roles associated with a name' do
      expect(@mult_corp_name_sub.corporate_name.role.roleTerm.map(&:text)).to eq(['lithographers.'])
    end

    it 'is able to identify dates associated with a name' do
      expect(@mult_pers_name_sub.personal_name.date.map(&:text)).to include('1818-1878')
    end
  end
  ...
  it 'has as many members as there are <subject> elements in the xml' do
    expect(@four_subjects.size).to eq(4)
    expect(@lcsh_subject.size).to eq(1)
  end

  it 'recognizes authority attribute on <subject> element' do
    %w[lcsh ingest lctgm].each do |a|
      expect(mods_record("<subject authority='#{a}'><topic>Ruler, English.</topic></subject>").subject.authority).to eq([a])
    end
  end

  context 'with <topic> child element' do
    before(:all) do
  ...
  context 'with <geographic> child element' do
    it 'is a NodeSet' do
    ...
    it 'does not include <geographicCode> element' do
      expect(@geo_code_subject.geographic.size).to eq(0)
    end
  end

  context 'with <temporal> child element' do
    before(:all) do
    ...
    it 'recognizes the date attributes' do
      expect(@temporal.encoding).to eq(['iso8601'])
      Mods::DATE_ATTRIBS.each do |a|
    ...
    it 'text should get element value' do
      expect(@lcsh_subject.temporal.map(&:text)).to eq(['500-1400'])
      expect(@temporal.map(&:text)).to eq(['20010203T040506+0700'])
    end
  end
  ...
  context 'with <genre> child element' do
    ...
    it 'text should get element value' do
      expect(@lcsh_subject.genre.map(&:text)).to eq(['Maps'])
      expect(@four_subjects.genre.map(&:text)).to include('Pictorial works')
    end
  end

  context 'with <geographicCode> child element' do
    it 'is a NodeSet' do
    ...
    it 'text should get element value' do
      expect(@geo_code_subject.geographicCode.map(&:text)).to eq(['f------'])
    end

    it 'recognizes authority attributes' do
    ...
    it 'recognizes the sanctioned authorities' do
      Mods::Subject::GEO_CODE_AUTHORITIES.each do |a|
    ...
    context 'with translated_value convenience method' do
      it 'is the translation of the code if it is a marcgac code' do
      ...
      it 'is the translation of the code if it is a marccountry code' do
      ...
      it 'is nil if the code is invalid' do
      ...
      it "is nil if we don't have a translation for the authority" do
  ...
  context 'with <titleInfo> child element' do
    before(:all) do
    ...
    it 'understands all attributes allowed on a <titleInfo> element' do
      Mods::TitleInfo::ATTRIBUTES.each do |a|
    ...
    it 'understands all immediate child elements allowed on a <titleInfo> element' do
      ...
      expect(@title_info.nonSort.map(&:text)).to eq(['The'])
    end

    it 'recognizes authority attribute on the <titleInfo> element' do
  ...
  context 'with <name> child element' do
    it 'understands all attributes allowed on a <name> element' do
      Mods::Name::ATTRIBUTES.each do |a|
    ...
    it 'understands all immediate child elements allowed on a <name> element' do
      Mods::Name::CHILD_ELEMENTS.each do |e|
    ...
    it 'recognizes authority attribute on the <name> element' do
      expect(mods_record("<subject>
  ...
  context 'with <hierarchicalGeographic> child element' do
    it 'recognizes authority attributes' do
      Mods::AUTHORITY_ATTRIBS.each do |a|
    ...
    it 'recognizes allowed child elements' do
      Mods::Subject::HIER_GEO_CHILD_ELEMENTS.each do |e|
  ...
  context 'with <cartographics> child element' do
    before(:all) do
    ...
    it 'recognizes allowed child elements' do
      Mods::Subject::CARTOGRAPHICS_CHILD_ELEMENTS.each do |e|
    ...
    it 'gets the number of populated <coordinates> elements for coordinates term' do
      @multi_carto.coordinates.size == 2
    end

    it 'is able to get the value of populated elements' do
      expect(@carto_scale.scale.map(&:text)).to eq(['[ca.1:90,000,000], [173'])
      expect(@carto_empties.coordinates.map(&:text)).to eq(['W1730000 W0100000 N840000 N071000'])
    end

    it 'gets the empty string for empty elements?' do
      expect(@carto_empties.projection.map(&:text)).to eq([''])
    end
  end

  context 'with <occupation> child element' do
    before(:all) do
    ...
    it 'text should get element value' do
      expect(@occupation.map(&:text)).to eq(['Migrant laborers'])
    end
