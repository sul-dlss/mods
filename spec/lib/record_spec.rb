...
describe 'Mods::Record' do
  context 'with from_str' do
    before(:all) do
      @mods_ng_doc_w_ns = mods_record('<note>default ns</note>')
    end

    it 'returns a mods record' do
      expect(@mods_ng_doc_w_ns).to be_a_kind_of(Mods::Record)
    end

    it 'has namespace aware parsing turned on by default' do
    ...
    it 'terminology should work with Record object defaults when mods string has namespaces' do
      expect(@mods_ng_doc_w_ns.note.map(&:text)).to eq(['default ns'])
    end
  ...
  context 'with from_url' do
    ...
    it 'raises an error on a bad url' do
  ...
  context 'with from_nk_node' do
    ...
    it 'terminology should work with Record object defaults when mods string has namespaces' do
      expect(@mods_ng_doc.title_info.title.map(&:text)).to eq(['boo'])
    end
  ...
  context 'when getting term values' do
    subject(:mods_rec) do
      mods_record(<<-XML)
    ...
    context 'with term_value (single value result)' do
      it 'returns nil if there are no such values in the MODS' do
        expect(mods_rec.term_value(:identifier)).to be_nil
      end

      it 'returns nil if there are only empty values in the MODS' do
        expect(mods_rec.term_value(:genre)).to be_nil
      end

      it 'returns a String for a single value' do
        expect(mods_rec.term_value(:abstract)).to eq('single')
      end

      it 'returns a String containing all values, with separator, for multiple values' do
        expect(mods_rec.term_value(:note)).to eq('mult1 mult2')
      end

      it 'works with an Array of messages passed as the argument' do
        expect(mods_rec.term_value([:subject, 'topic'])).to eq('topic1 topic2 topic3')
      end

      it 'takes a separator argument' do
        expect(mods_rec.term_value(:note, ' -|-')).to eq('mult1 -|-mult2')
      end
    end

    context 'with term_values (multiple values)' do
      it 'returns nil if there are no such values in the MODS' do
      ...
      it 'returns an array of size one for a single value' do
        expect(mods_rec.term_values(:abstract)).to eq(['single'])
      end

      it 'returns an array of values for multiple values' do
        expect(mods_rec.term_values(:note)).to eq(%w[mult1 mult2])
      end

      it 'works with an Array of messages passed as the argument' do
        expect(mods_rec.term_values([:subject, 'topic'])).to eq(%w[topic1 topic2 topic3])
      end

      it 'works with a String passed as the argument' do
        expect(mods_rec.term_values('abstract')).to eq(['single'])
      end

      it 'raises an error for an unrecognized message symbol' do
        expect do
          mods_rec.term_values(:not_there)
        end.to raise_error(ArgumentError,
                           'term_values called with unknown argument: :not_there')
      end

      it 'raises an error if the argument is an Array containing non-symbols' do
        expect do
          mods_rec.term_values([:subject,
                                mods_rec.subject])
        end.to raise_error(ArgumentError, /term_values called with Array containing unrecognized class:.*NodeSet.*/)
      end

      it "raises an error if the argument isn't a Symbol or an Array" do
        expect do
          mods_rec.term_values(mods_rec.subject)
        end.to raise_error(ArgumentError,
  ...
  context 'with convenience methods for accessing tricky bits of terminology' do
    context 'with title methods' do
    ...
    context 'with personal_names' do
      before(:all) do
        @pers_name = 'Crusty'
        @pers_role = 'creator'
      ...
      it 'does not include the role text' do
        record = mods_record("<name type='personal'><namePart>#{@pers_name}</namePart></name>")
        expect(record.personal_names.first).not_to match(@pers_role)
      end

      it 'prefers displayForm over namePart pieces' do
      ...
      it 'puts the family namePart first' do
    ...
    context 'with personal_names_w_dates' do
      ...
      it 'includes the date when it is available' do
      ...
      it 'is just the personal_name if no date is available' do
        expect(@given_family.personal_names_w_dates.first).to eq('Borges, Jorge Luis')
      end
    end

    context 'with corporate_names' do
      before(:all) do
        @corp_name = 'ABC corp'
      end
